#!/bin/bash
# Experiment 0014: Cursor (acp) in bb, real Wi-Fi cut. Every 20s snapshot the
# probe, the watcher state, the watcher log tail, and the three bb threads'
# status plus last two events.
# Usage: bash experiments/0014-capture.sh <minutes> <target-id> <control-id> <reference-id>
MIN=${1:-40}; TGT=${2:?target thread}; CTL=${3:?control thread}; REF=${4:?reference thread}
OUT=experiments/0014-captures; mkdir -p "$OUT"; LOG="$OUT/snapshots.log"
