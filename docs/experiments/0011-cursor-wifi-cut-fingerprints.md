...
(cmux, Terminal.app, bb ACP). David flipped Wi-Fi off by hand for 12.3 min.
...
- Three Cursor CLI sessions, model Auto, mid-essay when the cut hit:
  cmux surface:12 (`/tmp/oar-exp11-cmux`), Terminal.app `/dev/ttys010`
  (`/tmp/oar-exp11-terminal`), bb `thr_fat627yzkp` "0011 Cursor bb: undersea
  cables" (ACP session `72ca40ee-…`).
- Capture loop (`0011-capture.sh`) snapped pane text, bb events, store
  mtimes, hooks, and process count every 20s into `0011-captures/`.
...
- 22:33:42–43 watcher: cmux surface:12 and Terminal ttys010 `skip`
  `unknown_harness`. bb Cursor not enumerated (`idle`, and `acp-cursor` is
  out of `host_bb.PROVIDERS`)
- 22:33:44 collateral: `thr_other` ("other-thread",
  `other-project`, claude-code) `resume_sent ok`, Discord `sent=true`
...
- Hook: `stop` with `status: error` at that moment (`~/.example-hooks/cursor-hooks.jsonl`)
...
Same side effect as 0010. `other-thread` in `other-project` was
