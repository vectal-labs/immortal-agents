"""Pi coding-agent detector implementing the shared detector contract."""
...
def encode_cwd(cwd):
    """Absolute path -> Pi session folder name(s). /Users/me/code -> --Users-me-code--."""
    if not cwd:
        return []
    return [f"--{path.strip('/').replace('/', '-')}--" for path in cwd_variants(cwd)]
