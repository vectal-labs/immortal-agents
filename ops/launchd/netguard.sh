#!/bin/bash
# netguard: optional safety net after Incident 0001. Runs every 2 minutes.
# 1. Any DISABLED network service is re-enabled. This is intentionally
#    aggressive; use it only where disabled services are always unexpected.
# 2. Wi-Fi radio is turned back on only when an outage test left it off:
#    cut-armed.json exists and its deadline has passed. A radio turned off
#    without that file is never touched.
